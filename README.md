...
导出 OBJ、改名字、套材质，资产一多就要重复同一套操作。这里支持 **批量 OBJ（可选附带原点信息文本）**、**正则改名**（含重名时跳过/覆盖/加后缀）、以及 **一键同步为指定材质 / 批量应用材质 / 整理槽 / 清未使用**。

- 原点信息支持三种输出：`模型名: { -2.350247f, 0.003200f, 0.911799f }`、JSON 对象、CSV，方便开发直接复制使用。
- `Ctrl + Alt + M` 可弹出材质选择窗口，把选中网格对象统一替换为指定材质。
...
| **旋转快照与还原** | Alt+R 自动保存快照，Alt+Shift+R 还原；解决清除旋转后无法恢复的痛点。 |
| **建模模式饼图** | 长按 Tab 呼出精简模式饼图：物体、顶点编辑、边编辑、面编辑、姿态；短按 Tab 保持物体/编辑模式切换。 |
| **性能测试** | 创建 500 个带随机材质的立方体 + 随机运动，粗测本机/场景压力。 |
...
| **长按 Tab** | 打开 **建模模式饼图**：物体、顶点编辑、边编辑、面编辑、姿态；短按 Tab 仍为物体/编辑模式切换。 |
...
| 长按 `Tab` | 打开 **建模模式饼图**：物体、顶点编辑、边编辑、面编辑、姿态 |
| `Ctrl + M` | 镜像增强 |
| `Ctrl + F` | 批量重命名 |
| `Ctrl + Alt + M` | 同步选中对象为指定材质 |
| `Alt + R` | 清除旋转（增强：自动保存快照后归零） |
| `Alt + Shift + R` | 还原旋转快照 |
| 小键盘 `.` | 智能定位 |
